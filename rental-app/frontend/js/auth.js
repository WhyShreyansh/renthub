const BASE_URL = 'https://renthub-3-wc7z.onrender.com';
...
function initAuth() {
  ...
  if (token && user) {
    ...
    if (navAvatar) {
      if (user.avatar) {
        // ✅ FIXED (no localhost)
        navAvatar.src = `${BASE_URL}${user.avatar}`;
      } else {
        navAvatar.style.cssText =
          'background:var(--primary);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:14px;border-radius:50%;width:34px;height:34px;';
